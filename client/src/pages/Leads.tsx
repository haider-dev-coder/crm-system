    ...
    reader.onload = (evt) => {
      try {
        ...
        jsonData.forEach((row) => {
          // Helper function to convert values to strings (Excel may read as numbers)
          ...
          const leadData: Partial<InsertLead> = {
            // Note: date is auto-generated by the database, not imported
            ...
            price: toString(row["Price (AED)"] || row.price),
            ...
            source: toString(row["Bayut/Dubizzle"] || row.source),
            assignedTo: matchedUser?.id,
            pinnedNotes: toString(row["Pinned Notes"] || row.pinnedNotes),
            status: toString(row.Status || row.status || "new").toLowerCase(),
          };
